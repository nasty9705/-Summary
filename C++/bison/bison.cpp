...
enum lex
{
  l_NULL,
  l_INC_BEG, l_INC_END,
  l_INC, l_TOKEN, l_GR_BEG,
   l_GR_END, l_SEMICOLON,
   l_COLON, l_OR, l_OPEN,
  l_CLOSE, l_COM, l_STRING_I,
  l_STRING_CODE, l_STRING_LR,
   l_STRING_RR, l_ERR, l_FIN, l_TOKEN_OPEN
  };
...
class My_lexemes
{
    ...
    bool stool;
public:
    My_lexemes(FILE* f):file(f), current(START), column(1), row(0)
    {}
    vector<Lexema> parse()
    {
        return lexems;
...
public:
    Razbor(vector<Lexema> l):lexems(l){}
    ...
    friend void symbols_delete(Razbor& pars);
    friend void not_usable(Razbor& pars);
 
};
 
void symbols_delete(Razbor& pars)
{
    vector<string> tmp;
    bool changed = true;
...
int main (int argc, char* argv[])
{
 ...
 My_lexemes lexemes(file);
   if(lexemes.get_lexems()){  
Syntax synt(lexemes.parse());
if(synt.analize()){
Razbor p(lexemes.parse());
p.find();
}
}
    else
       cout << "Ошибка в лексеме" << endl;
